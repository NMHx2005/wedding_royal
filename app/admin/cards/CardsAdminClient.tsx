...
export default function CardsAdminClient({ cards: initial }: Props) {
  ...
  const filtered = cards.filter(
    (c) =>
      ...
      c.bride_name.toLowerCase().includes(q.toLowerCase()) ||
      c.groom_name.toLowerCase().includes(q.toLowerCase())
  );
  ...
  const deleteCard = async (id: string, slug: string) => {
    if (!confirm(`Xóa thiệp /${slug}? Thao tác không thể hoàn tác.`)) return;
    setLoading(id + "del");
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <input
          className="rounded-lg border border-neutral-200 px-3 py-2 text-sm outline-none focus:border-rose-300"
          placeholder="Tìm theo slug, tên..."
          value={q}
