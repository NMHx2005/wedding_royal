...
import { couplesShowcase, faqMehappy, MEHAPPY_ASSET } from "@/lib/data/mehappy-landing";
...
export function CacCapDoiClient({ faqItems = faqMehappy }: Props) {
  const [q, setQ] = useState("");
  const [page, setPage] = useState(1);

  const filtered = useMemo(() => {
    const needle = normalize(q.trim());
    if (!needle) return couplesShowcase;
    return couplesShowcase.filter((c) => {
      ...
      <div className="relative hidden min-h-[14rem] w-full overflow-hidden md:block md:min-h-[16rem]">
        ...
        <div className="relative z-10 mx-auto flex max-w-6xl flex-col justify-center px-4 py-12 md:min-h-[16rem] md:py-16">
          ...
          <p className="mt-3 max-w-lg text-sm leading-relaxed text-neutral-700 sm:text-base">
            Khám phá những thiệp cưới tuyệt đẹp được tạo bởi cộng đồng người dùng meWedding.
          </p>
        </div>
      </div>
